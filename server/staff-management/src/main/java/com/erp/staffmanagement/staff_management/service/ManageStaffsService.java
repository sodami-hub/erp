...
public class ManageStaffsService {
  ...
  public List<StaffInfoDTO> getStaffsForStatus(String status) throws Exception {
    ...
    return switch (status) {
      case "all" -> staffs;
      case "onDuty" ->
          staffs.stream().filter(object -> "402".equals(object.getWorkStatus())).collect(
              Collectors.toList());
      case "offDuty" ->
          staffs.stream().filter(object -> "405".equals(object.getWorkStatus())).collect(
              Collectors.toList());
      case "break" ->
          staffs.stream().filter(object -> "403".equals(object.getWorkStatus())).collect(
              Collectors.toList());
      case "waiting" ->
          staffs.stream().filter(object -> "401".equals(object.getWorkStatus())).collect(
              Collectors.toList());
